...
echo "🚀 Step 2: Transferring backend + built frontend to Azure VM..."
ssh -i "$KEY_PATH" "$VM_USER@$VM_HOST" "mkdir -p $TARGET_DIR/frontend-ui"
scp -r -i "$KEY_PATH" package.json package-lock.json src prisma "$VM_USER@$VM_HOST:$TARGET_DIR/"
scp -r -i "$KEY_PATH" frontend-ui/dist "$VM_USER@$VM_HOST:$TARGET_DIR/frontend-ui/"

echo "🔑 Step 3: Fetching DATABASE_URL from Key Vault for the migration step..."
...
echo "🔄 Step 4: Installing deps, generating Prisma client, applying migrations, restarting..."
