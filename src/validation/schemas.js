...
const dateValue = (label, missing) =>
  z
    ...
    .union([z.string(), z.number()], {
      error: absentOr(missing, `${label} must be a valid date`),
    })
...
const eventCreate = body({
  ...
  venueId: idValue({
    missing: missingEvent("venueId"),
    invalid: "That venue doesn't exist",
  }),
  ...
  .transform((event) => ({
    ...
    supply: event.supply
      ? {
          item: event.supply.item,
          quantity: event.supply.quantity ?? event.capacity,
        }
      : null,
  }));
...
const roomQuery = z.object({
  room: z
    .string({ error: "room query param is required" })
    .trim()
    .min(1, "room query param is required")
    .max(VARCHAR),
});
