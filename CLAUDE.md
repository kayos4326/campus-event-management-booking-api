...
- ✅ **GitHub** (requirement 9) — **pushed 2026-09-19** to
  ...
  - **Author email corrected before the first push, 2026-09-19.** Every commit was authored as
    ...
    commits were re-authored to `162141522+kayos4326@users.noreply.github.com` (the same person,
    the account that owns the repo) with `git filter-branch --env-filter`. Names, messages, dates,
    order and file contents are untouched — the root tree is byte-identical — but **every commit
    hash changed**. References to the old hashes were remapped (`06c45a5` → `5ee176d`,
    `01b4dcc` → `1e1f2ac`). The release directory `20260917-171608-6bdf478` on the VM keeps its
