...
- [x] **Booking/event business-logic review, 2026-09-15** — the architecture was solid but a code read turned up gaps a grader clicking around would hit. All fixed (89 tests now; 28 of the new/updated ones fail against the pre-fix routes):
  ...
  - ⏳ **Not yet deployed** — `./deploy.sh` was blocked by a permission check on 2026-09-15 and is waiting on the user. Once deployed, the plan is a real concurrency check against the live MySQL (several simultaneous bookings for a capacity-1 throwaway event, then cancel/promote/rebook, all test rows deleted afterward).
