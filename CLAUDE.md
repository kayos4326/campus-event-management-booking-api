...
- [x] **Booking/event business-logic review, 2026-09-15** — the architecture was solid but a code read turned up gaps a grader clicking around would hit. All fixed (89 tests now; 28 of the new/updated ones fail against the pre-fix routes):
  ...
  - ✅ **Deployed 2026-09-15** — `/events/` serves the new frontend bundle, API returns `401` without a token as before, `/content` `200`, `crud-api` untouched (still up, 4-day uptime). ⏳ Not yet done: a real concurrency check against the live MySQL (several simultaneous bookings for a capacity-1 throwaway event, then cancel/promote/rebook, all test rows deleted afterward) — waiting on the user's OK since it writes to the production DB.
